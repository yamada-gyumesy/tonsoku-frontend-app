...
- **掲載情報の「松のや」**: 名前（「とん速」）には入れない。iOS のサブタイトル・Play の短い説明
  （どちらも「松のや速報」）・説明文の 1 行目・プロモーション用テキストには**入れて出す**
  （ユーザーの判断: 「一旦出して、指摘されたら直す」）。gyumesy は名前・サブタイトルの「松屋」で
  App Store の 4.1(a) を 2 回受けているので、**App Store で 4.1(a) を受けたら、まずサブタイトルを
  「店舗限定メニューや㊙クーポン情報を爆速でお届け」に替える**（ユーザーが決めてある替えの文）。
  それでも受けたら、説明文の 1 行目とプロモーション用テキストから「松のや」を外す。
  Google Play は冒頭に出してよい
