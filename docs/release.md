...
- **掲載情報の「松のや」**: 名前（「とん速」）と iOS のサブタイトル（「店舗限定メニューやクーポン情報を
  爆速でお届け」）には入れない。Play の短い説明（「松のや速報」）・説明文の 1 行目・プロモーション用
  テキストには**入れて出す**（ユーザーの判断: 「一旦出して、指摘されたら直す」）。gyumesy は
  名前・サブタイトルの「松屋」で App Store の 4.1(a) を 2 回受けているので、**App Store で 4.1(a) を
  受けたら、説明文の 1 行目とプロモーション用テキストから「松のや」を外す**（外した形は
  「とんかつチェーン…」。git の履歴の #32 を参照）。Google Play は冒頭に出してよい
