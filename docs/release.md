...
- **掲載情報の「松のや」**: 名前（「とん速」）には入れない。サブタイトル・短い説明（「松のや速報」）・
  説明文の 1 行目・プロモーション用テキストには**入れて出す**（ユーザーの判断: 「一旦出して、
  指摘されたら直す」）。gyumesy は名前・サブタイトルの「松屋」で App Store の 4.1(a) を 2 回
  受けているので、**App Store で 4.1(a) を受けたら、まずサブタイトル → 説明文の 1 行目・
  プロモーション用テキストの順に「松のや」を外す**（外した形は「とんかつチェーン…」。
  git の履歴の #32 を参照）。Google Play は冒頭に出してよい
