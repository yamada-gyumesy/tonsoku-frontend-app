...
ストアの掲載情報は `ios/fastlane/metadata/` と `android/fastlane/metadata/`（日本語のみ）。**名前に「松のや」を入れない**（gyumesy は「松屋」で App Store の 4.1(a) を 2 回受けた）。サブタイトル・説明文の冒頭には入れて出し、指摘されたら替える（`docs/release.md` の「提出前の確認」）。非公式であることは説明文に書く。
